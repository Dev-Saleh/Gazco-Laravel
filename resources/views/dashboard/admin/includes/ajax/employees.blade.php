...
<script>
 ...
 // ########################## ( Employees SECTION ) ##############################
    ...
    // Start Search Employees By Ajax 

        $(document).on('keyup', '#search-dropdown', function(e) {
            e.preventDefault();
            var formData = new FormData($('#emplSearchForm')[0]);
            $.ajax({
                type: 'Post',
                enctype: 'multipart/form-data',
                        ...
                        $.each(data.resultSearch, function (key , resultSearch) {
                         $('#fetchEmployees').prepend('<tr  class="offerRow'+resultSearch.id+' bg-white hover:scale-95 transform transition-all ease-in">\
                              ...
                              <td class="p-5 flex space-x-2">\
                                ...
                                <a href="#" empId="'+resultSearch.id+'"   class="employeeEdit text-yellow-400 hover:text-yellow-600  mx-2">\
                                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">\
                                    <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />\
                                  </svg>\
                                </a>\
