...
// ########################## ( Stations SECTION ) ##############################
    ...
    // Start fetch All Station 

      function fetchstation()
        {
            $.ajax({
                    type: 'get',
                    url: "{{route('station.show_all_Data')}}",
                    dataType:"json",

                    success: function (data) { 
                        
                    $('#fetch_Allstation').html("");
                      $.each(data.stations, function (key , station) {
                        $('#fetch_Allstation').append('<tr class="offerRow'+station.id+' class="bg-gray-100 hover:scale-95 transform transition-all ease-in">\
