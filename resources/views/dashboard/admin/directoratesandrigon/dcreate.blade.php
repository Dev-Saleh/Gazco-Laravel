...
<!--End Deleteing Directorate By Ajax -->
<!--Start Show Directorate By Ajax -->
<script>
function showDataDirectorate()
{
      $.ajax({
             type: 'GET',
             url:"{{route('directorate.show',0)}}",
    success: function (data) {
      $('#directorate').html("");
    $.each(data.directorate, function (key, val) {
        $('#directorate').append('<tr class="offerRow'+val.id+'" class="bg-gray-100 hover:scale-95 transform transition-all ease-in">\
                              <td class="p-3">'+val.id+'</td>\
                             <td class="p-3 text-center">'+val.directorate_name+'</td>\
                              <td class="p-3 flex justify-evenly ">\
                                <a href="#"  directorate="'+val.id+'"  class="directorate_delete btn btn-danger" class="text-gray-400  hover:text-red-400  ">\
                                  <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">\
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />\
                                  </svg>\
                                </a>\
                                <a href="#"  directorate="'+val.id+'" id="directorate_edit" class="text-gray-400 hover:text-yellow-100  ">\
                                  <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">\
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />\
                                  </svg>\
                                </a>\
  ...
  <script>
      $(document).on('click', '#directorate_edit', function (e) {
            ...
            $.ajax({
              ...
              success: function (data) {
                    if (data.status == true) {
                    $('#directorate_name').val(data.data.directorate.directorate_name);
                    $('#directorate_id').val(data.data.directorate.id);
